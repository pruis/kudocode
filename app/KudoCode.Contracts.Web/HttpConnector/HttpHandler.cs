...
namespace KudoCode.Contracts.Web
{
	public class HttpHandler
	{
		...
		//where TRequestDto : IApiRequestDto
		{
			...
			else
			{
				if (_configuration["ApiBaseUrl"] == null)
					throw new Exception("FATAL: ApiBaseUrl CAN NOT BE NULL Or serviceDestination url not set");
		...
		public IApiResponseContextDto<TResponseDto> Post<TRequestDto, TResponseDto>(TRequestDto dto, string serviceDestination = "", CancellationToken? cancellationToken = null)
			where TRequestDto : IApiRequestDto
		{
			var hostUrl = string.Empty;
