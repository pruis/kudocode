...
namespace KudoCode.Contracts.Web
{
    ...
    public class ApiPostHandler<TRequestDto, TResponseDto> where TRequestDto : IApiRequestDto
    {
        ...
        public async Task<IApiResponseContextDto<TResponseDto>> Post(TRequestDto dto, string serviceDestination = "")
        {
            IApiResponseContextDto<TResponseDto> result = null;
            try
            {
                result = _httpHandler.Post<TRequestDto, TResponseDto>(dto, serviceDestination);
                return result;
            }
            catch (Exception e)
            {
